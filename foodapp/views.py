...
from .ai_utils import generate_recipe_recommendations
...
def ai_recipe_recommendation(request):
    # Require your existing custom user session
    user_id = request.session.get("user_id")
    is_admin = request.session.get("is_admin")

    if not user_id or is_admin:
        messages.error(request, "Please login as a user first.")
        return redirect("login")
    ...
    if request.method == "POST":
        ...
        if not ingredients:
            ...
            try:
                recommendation = generate_recipe_recommendations(ingredients)
            except Exception as exc:
                print("Gemini Recipe Error:", exc)
                error = "Unable to generate recipes right now. Please try again."

    return render(
        request,
        "user/ai_recipe_recommendation.html",
        {
            "ingredients": ingredients,
